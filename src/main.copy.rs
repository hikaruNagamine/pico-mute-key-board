...
fn body() -> Result<(), usb_device::UsbError> {
    ...
    loop {
        ...
        // debug!("change keycodes!!!");
        // push_key_event(key_repo).ok().unwrap_or(0);
        // last_keycodes = key_repo.keycodes;
        // }

        // let init_key_repo2 = KeyboardReport {
        //     modifier: 0,
        //     reserved: 0,
        //     leds: 0,
        //     keycodes: [0x05 /* A */, 0, 0, 0, 0, 0],
        // };
        // push_key_event(init_key_repo2).ok().unwrap_or(0);
...
impl<'a> KeyScanner<'a> {
    ...
    fn scan_matrix(&mut self) -> [[bool; 2]; 3] {
        ...
        for (y, row) in matrix.iter_mut().enumerate() {
            ...
            delay_ms(10); // Wait a bit to propagate the voltage
...
pub fn push_key_event(report: KeyboardReport) -> Result<usize, usb_device::UsbError> {
    ...
    cortex_m::interrupt::free(|_| unsafe {
        USB_HID_KEYBOARD
            .as_mut()
            .map(|hid_keyboard| hid_keyboard.push_input(&report))
    })
    .unwrap()
}
